...
lib = CDLL("/home/nvidia/darknet/libdarknet.so", RTLD_GLOBAL) ##replsce it with path in pc
...
if __name__ == "__main__":
	...
	pub = rospy.Publisher('obj_rec', String, queue_size=10)
	pub2 = rospy.Publisher('face_rec', String, queue_size=10)
	rospy.init_node('obj_and_face', anonymous=True)
	#rate = rospy.Rate(10) # 10hz
		...
		if len(re) > 0:
			for i in range(0, int(len(re)/3)):
				...
				if re[3*i].decode("utf-8") == "person":
					...
					if len(names) > 0:
						rospy.loginfo(str(names[0]))
						pub2.publish(str(names[0]))
					print("names:",names)
#		for name in re:
#			print("name:", name)
		#rospy.loginfo(str(re))
		#pub.publish(str(re))
		#rate.sleep()
		#talker.talker(str(fps))
		#if re is not []:
#			print(re)
			#talker(re)
		#c = c+1
		#cv2.imshow("image", i)mage)
		cv2.waitKey(1)
